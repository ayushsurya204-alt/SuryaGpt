...
const Sidebar = ({isMenuOpen,setIsMenuOpen}) => {
  ...
  transistion-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen &&
   'max-md:-translate-x-full'}`}>
      {/*logo*/}
      <img src={theme === 'dark' ? assets.logo_full : assets.logo_full_dark} 
      className='w-full max-w-48'  alt="" />
    ...
    >
     ...
     dark:border-white/15 rounded-md cursor-pointer hover-scale-103 transistion-all'>
     ...
     dark:border-white/15 rounded-md cursor-pointer hover-scale-103 transistion-all'>
      ...
      <div className='flex flex-col text-sm'>
        ...
        <p className='text-xs text-gray-400'>Purchase credits to use suryagpt</p>
      </div>
     </div>
     ...
     dark:border-white/15 rounded-md'>
      ...
      <label className='relative inline-flex cursor-pointer'>
        <input onChange={()=>setTheme(theme === 'dark' ? 'light' : 'dark')}
         ...
         transistion-transfrom peer-checked:translate-x-4'></span>
      </label>
     </div>
